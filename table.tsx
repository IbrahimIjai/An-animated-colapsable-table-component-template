...
export default function DemoWithControls() {
  ...
  let [duration, setDuration] = useState<number>(0.25);
  let [expand, setExpand] = useState(false);
  let [count, setCount] = useState(0);

  const handler = async (e: any) => {
    let realVal = e.target.value;
    let _duration = realVal.toNumber();
...
function ResizablePanel({ children, animationName, duration }:props) {
